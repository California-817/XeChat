...
namespace XeChat
{
    ...
    MyModule::MyModule()
        : Module("XeChat", "libXeChat.so", "1.0.0")
    {
    }
    // 命令行参数解析前后执行
...
extern "C"
{
    ...
    {
        Xten::Module *mod = new XeChat::MyModule();
        XTEN_LOG_INFO(XeChat::g_logger) << "CreateModule: " << mod;
        return mod;
    }
    void DestoryModule(Xten::Module *ptr)
    {
        delete ptr;
        XTEN_LOG_INFO(XeChat::g_logger) << "DestoryModule: " << ptr;
    }
}